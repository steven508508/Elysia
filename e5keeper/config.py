...
def load_settings(
    ...
    *,
    allow_empty_accounts: bool = False,
    skip_accounts: bool = False,
) -> Settings:
    """載入設定。

    allow_empty_accounts：E5_ACCOUNTS 還沒建立時不報錯（第一次授權會用到）。
    skip_accounts：連解析都不做，直接當成沒有帳號。
        給「帳號 JSON 壞掉，但還是要把指令通道撐起來」的降級路徑用 ——
        指令通道正是你用來發現東西壞掉的管道，不能跟著一起死。
    """
    cfg = load_config(config_path)
    raw_accounts = os.environ.get("E5_ACCOUNTS", "")
    if skip_accounts or (allow_empty_accounts and not raw_accounts.strip()):
        accounts: list[Account] = []
    else:
